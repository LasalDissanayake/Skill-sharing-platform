...
const LearningTab = () => {
  ...
  useEffect(() => {
    ...
    const fetchLearningPlans = async () => {
      ...
      try {
        ...
        const data = await response.json();
        if (isMounted) {
          console.log('Fetched user learning plans:', data);
  ...
  const handleWeekStatusChange = async (planId, weekIndex, currentStatus) => {
    ...
    try {
      ...
      const originalPlans = learningPlans.map((plan) => {
        if (plan.id === planId) {
      ...
      setLearningPlans(originalPlans);
      if (selectedPlan && selectedPlan.id === planId) {
  ...
  const styles = {
    ...
    breadcrumbActive: "text-indigo-600 font-medium text-sm flex items-center gap-1",
                ...
                <div>
                  ...
                  <div className="flex justify-between items-center mb-4">
                    ...
                    <span className="text-gray-600 font-medium">
                      Progress: {calculateProgress(selectedPlan.weeks)}%
                    </span>
                  </div>
                  ...
                  <div className={styles.progressBar}>
                    <div 
                      ...
                      style={{ width: `${calculateProgress(selectedPlan.weeks)}%` }}
                    ></div>
                  </div>
                    ...
                    {selectedPlan.weeks.map((week, index) => (
                      ...
                      >
                        ...
                        <div>
                          ...
                          <p className={styles.weekDescription}>{week.description || 'No description provided for this week.'}</p>
                        </div>
                      </div>
                    ))}
                  </div>
